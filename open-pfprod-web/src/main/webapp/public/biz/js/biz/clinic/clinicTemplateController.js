layui.config({
    ...
    function zTreeOnClick(event, treeId, treeNode) {
        v_idDemoCa = treeNode.id
        _tableReload(v_idDemoCa);
    };
    ...
    function addTreeNode(name) {
        ...
        // ajax add
        layer.load(2);
        var url = basePath + '/pf/r/clinic/template/classify/add';
        var bizData = {};
    ...
    function onRename(e, treeId, treeNode, isCancel) {
        ...
        var url = basePath + '/pf/r/clinic/template/classify/edit';
        var bizData = {};
        bizData.idDemoCa = treeNode.id;
    ...
    function removeTreeNode() {
        ...
        if (nodes && nodes.length > 0) {
            ...
            layer.confirm(msg, {
                ...
                var url = basePath + '/pf/r/clinic/template/classify/del';
                var bizData = {};
    ...
    $(document).ready(function () {
        layer.load(2);
        $.ajax({
            url: basePath + '/pf/r/clinic/template/classify/tree',
            type: 'post',
            dataType: 'json',
            contentType: "application/json",
            success: function (data) {
    ...
    table.render({
        ...
        //, url: basePath + '/pf/p/clinic/template/list'
        , limit: 15
        //, even: true
        , limits: [15, 30, 100]
        , page: true
    ...
    var _tableReload = function (idDemoCa, name) {
    ...
    $('#add').on('click', function () {
        ...
        var bizData = {};
        bizData.idDemoCa = treeNode.id;
        bizData.idDemoCaText = treeNode.name;
    ...
    var _addOrEdit = function (formType, currentEditData) {
        if (formType == 'add') {
            common.open('新增模板', basePath + '/pf/p/clinic/template/form?formType=' + formType, 440, 270, _successFunction(currentEditData));
        } else {
            common.open('编辑模板', basePath + '/pf/p/clinic/template/form?formType=' + formType, 440, 270, _successFunction(currentEditData));
        }
    };
    ...
    var _delQuestion = function (currentData) {
        var url = basePath + '/pf/r/clinic/template/del';
        ...
        data.list = reqData;
        data.status = '1';
        layer.confirm('确定删除模板部位描述' + messageTitle + '么？', {
            title: '删除提示',
            resize: false,
            btn: ['确定', '取消'],
            btnAlign: 'c',
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.code != 0) {
                        ...
                        _tableReload(currentData[0].idDemoCa);
                        return true;
                    }
                },
                error: function () {
    ...
    table.on('tool(templateTableFilter)', function (obj) {
        ...
        if (obj.event === 'editTemplate') {
            _editTemplate(data.name, data.idDemo);
        }
    });
    ...
    form.on('submit(templateSearchFilter)', function (data) {
        _tableReload(v_idDemoCa, data.field.name);
    });
