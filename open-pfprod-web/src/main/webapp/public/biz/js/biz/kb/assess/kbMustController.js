layui.config({
    ...
    $(document).ready(function () {
        if (previewFlag == '1') {
            var formIdArr = new Array('add', 'save', 'itemName', 'scoreEva', 'sdEvaType', 'idDieText');
    ...
    function loadInfo() {
        ...
        table.render({
            ...
            , url: basePath + '/pf/p/kb/assess/must/list'
            , where: {
                idEvaCase: idEvaCase,
                cdEvaAsse: cdEvaAsse
            }
    ...
    var _postReload = function (bizData) {
        //layer.load(2);
        $.ajax({
            url: basePath + '/pf/r/kb/assess/must/list',
            type: 'post',
            dataType: 'json',
            contentType: "application/json",
        ...
        if (obj.event === 'del') {
            ...
            if (obj.data.idEvaCaseItemList != null) {
                layer.confirm('真的要删除【' + data.desText + '】么？', function (index) {
                    ...
                    //向服务端发送删除指令
                    if (obj.data.idEvaCaseItemList != null) {
                        var url = basePath + '/pf/r/kb/assess/must/del';
    ...
    form.on('submit(saveAnswer)', function (data) {
        ...
        var url = basePath + '/pf/r/kb/assess/must/save';
